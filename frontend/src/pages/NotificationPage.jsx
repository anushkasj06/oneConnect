...
const NotificationsPage = () => {
	...
	const { data: notifications, isLoading } = useQuery({
		...
		queryFn: async () => {
			...
			return response.data; // Ensure data extraction
		},
	});
	...
	const renderNotificationContent = (notification) => {
		...
		switch (notification.type) {
			case "like":
				return (
					<span>
						<strong>{user?.name || "Someone"}</strong> liked your post
					</span>
				);
			case "comment":
				return (
		...
		return (
			...
			>
				...
				<div className='flex-1 overflow-hidden'>
					<p className='text-sm text-gray-600 truncate'>{relatedPost?.content || "View post"}</p>
				</div>
