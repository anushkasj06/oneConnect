...
const EducationSection = ({userData, isOwnProfile, onSave}) => {
  ...
  return (
    <div  className="bg-white shadow rounded-lg p-6 mb-6">
      ...
      {educations.map((edu)=>(
        <div key={edu._id} className="mb-4 flex items-center justify-between item-start">
          <div className="flex items-center">
            <School size={20} className="mr-2 mt-1"/>
            <div>
              <h3 className="font semi-bold">{edu.fieldOfStudy}</h3>
              <p className="text-gray-600" >{edu.school}</p>
              <p className="text-gray-500">{edu.startYear} - {edu.endYear || "Present"}</p>
            </div>
          </div>
          {isEditing && (
            <button
